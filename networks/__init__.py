...
def define_net(opt):
    ...
    elif which_model.find('INPAINT_V1') >= 0:
        from .inpaint_v1_arch import Inpaint_v1
        net = Inpaint_v1(in_channels=opt["in_channels"], out_channels=opt["out_channels"], num_features=opt["num_features"], num_steps=opt["num_steps"], act_type=opt["act_type"], norm_type=opt["norm_type"], residual_blocks=opt["residual_blocks"], compress_mode=opt["compress_mode"], fusion_mode=opt["fusion_mode"], use_unsup3d=opt["use_unsup3d"])
    elif which_model.find('INPAINT_V2') >= 0:
        from .inpaint_v2_arch import Inpaint_v2
        net = Inpaint_v2(inputChannels=4, outputChannels=4)
    elif which_model.find('PR') >= 0:
        from .pr_arch import PRNet
        net = PRNet(in_channels=4, out_channels=opt["out_channels"], num_features=opt["num_features"], num_steps=opt["num_steps"], act_type=opt["act_type"], norm_type=opt["norm_type"], residual_blocks=opt["residual_blocks"], compress_mode=opt["compress_mode"], unet_mode=opt["unet_mode"], iib_model=opt["iib_model"], context_attn=opt["context_attn"])
    else:
